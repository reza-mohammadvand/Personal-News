...
function render(){
 const filtered=state.articles.filter(a=>{const age=(Date.now()-new Date(a.published).getTime())/864e5;const text=`${a.title} ${a.summary} ${(a.tags||[]).join(' ')}`.toLowerCase();return age<=state.days+.25&&(state.source==='all'||a.source===state.source)&&(state.topic==='all'||a.topic===state.topic)&&(!state.query||text.includes(state.query.toLowerCase()))});
...
function card(a){const tags=(a.tags||[topicNames[a.topic]]).slice(0,3).map(t=>`<span class="tag">${esc(t)}</span>`).join('');const media=a.image?`<img class="card-image" src="${esc(a.image)}" alt="" loading="lazy" referrerpolicy="no-referrer" onerror="this.outerHTML='<div class=&quot;image-fallback&quot;>◌</div>'">`:`<div class="image-fallback">◌</div>`;return`<article class="card">${media}<div class="card-body"><div class="meta"><span class="source">${esc(a.source)}</span><time datetime="${esc(a.published)}">${relativeDate(a.published)}</time></div><h3>${esc(a.title)}</h3><p class="summary">${esc(a.summary||'برای مطالعه توضیحات کامل، وارد منبع اصلی خبر شوید.')}</p><div class="tags">${tags}</div><a class="card-link" href="${esc(a.link)}" target="_blank" rel="noopener noreferrer">مطالعه در منبع اصلی <span>←</span></a></div></article>`}
function bind(){
